...
Keep explanationEn and explanationNp concise, precise, and highly professional. Include actual figures in explanations.`;
  ...
  if (path === "/api/ai/chat" && method === "POST") {
    ...
    try {
      const functionDeclarations = [
        ...
        { name: "fill_data", description: "Navigate to the data input section and auto-fill form fields with suggested values for the assigned office. Use this when the user wants to update or fill indicator data.", parameters: { type: "OBJECT", properties: { values: { type: "OBJECT", description: "Optional key-value pairs of field names and suggested numeric values to fill" } }, required: [] } },
      ];
      const contents = [
      ...
      const data = await geminiGenerate(env, {
        contents,
        systemInstruction: {
          parts: [{ text: "You are an AI assistant for a Department of Roads dashboard application. You can answer questions, and perform actions like opening pages, printing, generating reports, showing menus, changing volume, and filling data input forms. When the user wants to update or fill indicator data, use the fill_data tool with suggested numeric values based on realistic progress updates." }]
        },
  ...
  if (path === '/api/verify-session' && method === 'POST') {
    ...
    const token = auth.replace('Bearer ', '');
    try {
      const verifyRes = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyCrrGZbhk2BF9Nfm4K8Osd_U7-YJLAvDbw`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ idToken: token }),
      });
      if (!verifyRes.ok) return withCors(Response.json({ error: 'Invalid token' }, { status: 401 }));
      const data = await verifyRes.json();
      const email = data.users?.[0]?.email;
      if (!email) return withCors(Response.json({ error: 'Invalid token' }, { status: 401 }));
      return withCors(Response.json({ email, role: email === 'banjays@gmail.com' ? 'superadmin' : 'unknown' }));
    } catch (error: any) {
      return withCors(Response.json({ error: error.message || 'Failed to verify session' }, { status: 500 }));
    }
  }
  ...
  if (path === '/api/admin/create' && method === 'POST') {
    ...
    try {
      ...
      const session = await sessionRes.json();
      if (!session.email) return withCors(Response.json({ error: 'Unauthorized' }, { status: 401 }));
      if (session.role !== 'superadmin') return withCors(Response.json({ error: 'Only superadmin can perform this action' }, { status: 403 }));
      const body = await request.json();
      const { email, role } = body;
      if (!email || !role) return withCors(Response.json({ error: 'Email and role are required' }, { status: 400 }));
      return withCors(Response.json({ success: true, action: 'created' }));
    } catch (error: any) {
      return withCors(Response.json({ error: error.message || 'Failed to process admin' }, { status: 500 }));
    }
  }
  ...
  if (path === '/api/admin/notifications/send' && method === 'POST') {
    ...
    try {
      ...
      const session = await sessionRes.json();
      if (session.role !== 'superadmin') return withCors(Response.json({ error: 'Forbidden' }, { status: 403 }));
      const body = await request.json();
      if (!body.message || typeof body.message !== 'string') return withCors(Response.json({ error: 'Message is required' }, { status: 400 }));
      return withCors(Response.json({ success: true, sentTo: 0 }));
    } catch (error: any) {
      return withCors(Response.json({ error: error.message || 'Failed to send' }, { status: 500 }));
    }
  }
