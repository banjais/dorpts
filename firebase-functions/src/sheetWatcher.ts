...
function parsePublishedDashboard(csv: string): ParsedSheet {
  const lines = csv
    .split("\n")
    .map((l) => l.trim())
    .filter((l) => l.length > 0);
...
// --- Core watcher -----------------------------------------------------------
export async function checkAndSnapshotFromSheet(): Promise<{ snapshotted: boolean; lastUpdateDate?: string; reason?: string }> {
  try {
    const res = await fetch(DASHBOARD_CSV);
    if (!res.ok) return { snapshotted: false, reason: `Dashboard fetch failed: ${res.status}` };
