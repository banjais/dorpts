...
async function verifySuperadmin(token: string): Promise<{ email: string } | null> {
  try {
    const adminAuth = getAuth(getAdminApp());
    const decoded = await adminAuth.verifyIdToken(token);
    if (decoded.email !== "banjays@gmail.com") return null;
...
Keep explanationEn and explanationNp concise, precise, and highly professional. Include actual figures in explanations (e.g. "Progress dropped from 364 to 100...").`;
  ...
  app.post("/api/ai/chat", async (req, res) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction:
            "You are an AI assistant for a dashboard application. You can answer questions, and perform actions like opening pages, printing, generating reports, showing menus, and changing volume. Use tools to perform actions when asked.",
  ...
  app.post("/api/admin/create", express.json(), async (req, res) => {
    const authHeader = req.headers.authorization;
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        await docRef.set({ email, role, updatedAt: new Date() }, { merge: true });
        return res.json({ success: true, action: "updated", id: docRef.id });
      }
      ...
      return res.json({ success: true, action: "created", id: docRef.id });
    } catch (error: any) {
      console.error("Admin create/update error:", error);
      res.status(500).json({ error: error.message || "Failed to process admin" });
    }
  });
