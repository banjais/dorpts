                     ...
                     <div className="space-y-2">
                      ...
                      <div className="bg-slate-50 dark:bg-slate-950/30 rounded-3xl p-6 border border-slate-100 dark:border-white/5 max-w-lg mx-auto">
                        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                          {language === 'en' 
                            ? 'Quick guide to navigating the dashboard, reading indicators, and using built-in tools including the AI Assistant for data input.'
                            : 'ड्यासबोर्ड नेभिगेट गर्न, सूचकहरू पढ्न र डाटा इनपुटका लागि एआई सहायक सहित अन्तर्निर्मित उपकरणहरू प्रयोग गर्न छिटो गाइड।'}
                        </p>
                      </div>

                      {/* Role-based access info */}
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-lg mx-auto">
                        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-500/20 rounded-2xl p-3 text-center">
                          <Crown size={16} className="text-amber-500 mx-auto mb-1.5" />
                          <p className="text-[9px] font-black text-amber-700 dark:text-amber-400 uppercase tracking-wider">
                            {language === 'en' ? 'Superadmin' : 'सुपरएडमिन'}
                          </p>
                          <p className="text-[8px] text-slate-500 dark:text-slate-400 mt-0.5">
                            {language === 'en' ? 'Full control' : 'पूर्ण नियन्त्रण'}
                          </p>
                        </div>
                        <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-500/20 rounded-2xl p-3 text-center">
                          <ShieldCheck size={16} className="text-indigo-500 mx-auto mb-1.5" />
                          <p className="text-[9px] font-black text-indigo-700 dark:text-indigo-400 uppercase tracking-wider">
                            {language === 'en' ? 'Admin' : 'प्रशासक'}
                          </p>
                          <p className="text-[8px] text-slate-500 dark:text-slate-400 mt-0.5">
                            {language === 'en' ? 'Data input + AI' : 'डाटा + एआई'}
                          </p>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-center">
                          <Users size={16} className="text-slate-400 mx-auto mb-1.5" />
                          <p className="text-[9px] font-black text-slate-600 dark:text-slate-300 uppercase tracking-wider">
                            {language === 'en' ? 'Viewer' : 'दर्शक'}
                          </p>
                          <p className="text-[8px] text-slate-500 dark:text-slate-400 mt-0.5">
                            {language === 'en' ? 'View only' : 'हेर्ने मात्र'}
                          </p>
                        </div>
                      </div>
