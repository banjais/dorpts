...
function MainAppContent() {
  ...
  useEffect(() => {
    ...
    if (appSettings.copyrightEn) APP_TITLES.copyright.en = appSettings.copyrightEn;
    if (appSettings.copyrightNp) APP_TITLES.copyright.ne = appSettings.copyrightNp;
    if (appSettings.officeBrandingEn) APP_TITLES.officeBranding.en = appSettings.officeBrandingEn;
    if (appSettings.officeBrandingNp) APP_TITLES.officeBranding.ne = appSettings.officeBrandingNp;
  ...
  }, [syncInterval, autoSyncEnabled, fetchData, accessToken]);

  useEffect(() => {
    const handleSyncChange = () => {
      const interval = localStorage.getItem("syncInterval");
      setSyncInterval(interval ? parseInt(interval) : 5);
    };
    const handleAutoSyncChange = () => {
      const enabled = localStorage.getItem("autoSyncEnabled");
      setAutoSyncEnabled(enabled !== "false");
    };
    window.addEventListener("syncIntervalChanged", handleSyncChange);
    window.addEventListener("autoSyncChanged", handleAutoSyncChange);
    return () => {
      window.removeEventListener("syncIntervalChanged", handleSyncChange);
      window.removeEventListener("autoSyncChanged", handleAutoSyncChange);
    };
  }, []);
  ...
  // Removed auto-speak on app start - users should control when they hear audio
    ...
    try {
      ...
      if (!isOfflineFallback) {
        try {
          ...
          // Silent fallback to standard getDoc/getDocs
        }
      }

      // Fallback individually for any missing snapshot
      try {
        if (!metadataSnap) {
          metadataSnap = await getDoc(doc(db, "metadata", "current")).catch(() => null);
        }
        if (!indicatorsSnap) {
          indicatorsSnap = await getDocs(query(collection(db, "indicators"), orderBy("id"))).catch(() => null);
        }
        if (!historySnap) {
          historySnap = await getDocs(query(collection(db, "updates_history"), orderBy("id", "desc"))).catch(() => null);
        }
      } catch (_) {
        // Suppress offline error during fallback
      }
      ...
      if (metadataSnap?.exists()) {
        ...
        try {
          const cachedMeta = localStorage.getItem("dor_metadata_cache");
          if (cachedMeta) {
      ...
      if (indicatorsSnap && !indicatorsSnap.empty) {
        ...
        try {
          const cachedInds = localStorage.getItem("dor_indicators_cache");
          if (cachedInds) {
            const parsed = JSON.parse(cachedInds);
    ...
    const unsubIndicators = onSnapshot(
      ...
      (snap) => {
        if (!snap.empty) {
          ...
          // Firestore is empty — prefer cached sheet data over hardcoded defaults
          let cached: Indicator[] = [];
          try {
            const raw = localStorage.getItem("dor_indicators_cache");
            if (raw) cached = JSON.parse(raw);
          } catch (_) {
            // ignore
          }
          const fallbackList = cached.length > 0 ? cached : DEFAULT_INDICATORS;
          setIndicators(fallbackList);
          if (fallbackList.length > 0) {
            const colRef = collection(db, "indicators");
            const writes = fallbackList.map((ind) =>
              setDoc(doc(colRef, ind.id), {
                ...ind,
      ...
      (err) => {
        ...
        setIndicators(cached.length > 0 ? cached : DEFAULT_INDICATORS);
        setLoading(false);
      },
    );
  ...
  useEffect(() => {
    ...
    const handleAIFillData = (e: any) => {
      setIsAIAssistantOpen(false);
      setTimeout(() => {
        handleMainViewChange('admin');
        // AdminDashboard will receive values via a separate custom event
        setTimeout(() => {
          window.dispatchEvent(new CustomEvent('ai:fill_data_request', { detail: { values: e.detail?.values || {} } }));
        }, 50);
      }, 50);
    };
    ...
    window.addEventListener("ai:fill_data", handleAIFillData);
    ...
    return () => {
      ...
      window.removeEventListener("ai:fill_data", handleAIFillData);
    };
  }, [filteredIndicators]);
  ...
  useEffect(() => {
    if (prevIndicatorsRef.current.length > 0 && indicators.length > 0) {
      const anomalies: string[] = [];
      indicators.forEach(ind => {
        const prev = prevIndicatorsRef.current.find(p => p.id === ind.id);
        if (prev) {
          const prevPct = prev.annualTarget > 0 ? (prev.annualProgress / prev.annualTarget) * 100 : 0;
          const currPct = ind.annualTarget > 0 ? (ind.annualProgress / ind.annualTarget) * 100 : 0;
          const change = currPct - prevPct;
          if (change < -10) {
            anomalies.push(`${language === 'en' ? ind.nameEn || ind.name : ind.name} (${Math.round(Math.abs(change))}% ${language === 'en' ? 'drop' : 'गिरावट'})`);
          }
        }
      });
      if (anomalies.length > 0) {
        const anomalyMsgEn = `AI Alert: ${anomalies.slice(0, 3).join(', ')} ${anomalies.length > 3 ? `and ${anomalies.length - 3} more` : ''} ${anomalies.length === 1 ? 'has' : 'have'} dropped significantly.`;
        const anomalyMsgNp = `एआई सूचना: ${anomalies.slice(0, 3).join(', ')} ${anomalies.length > 3 ? `र ${toNepaliNumerals(anomalies.length - 3)} अन्य` : ''} मा उल्लेखनीय गिरावट आएको छ।`;
        addToast(anomalyMsgNp, anomalyMsgEn, "warning", 8000);
      }
    }
...
<Overview
                          ...
                          {mainView === "detailed-gallery" && (
                            <ErrorBoundary
                              fallback={
                                <div className="p-8 text-center">
                                  <p className="text-sm text-slate-500 dark:text-slate-400">
                                    Detailed Gallery temporarily unavailable.
                                  </p>
                                </div>
                              }
                            >
            ...
            <>
              {(isAdmin || isSuperadmin) && (
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                   onClick={() => handleMainViewChange('superadmin')}
                  className="px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-[10px] font-black text-slate-700 dark:text-slate-300 shadow-sm hover:border-indigo-300 dark:hover:border-indigo-700 transition-all flex items-center gap-1.5"
...
export default function App() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('dor_') && key !== 'language' && key !== 'dor_app_version') {
